# ChatFlow360 - Changelog Archive: v0.3.0 – v0.3.20
...
- **Fix: bidirectional translate** — `POST /api/translate` now groups texts by translation direction (`from-to` key) and makes parallel OpenAI calls per group. Previously used the first item's `from`/`to` for ALL items in the batch, causing wrong translations when mixing EN→ES and ES→EN in one request.
- **Starter questions in bulk translate** — Welcome Screen "Translate empty fields" button now includes starter questions when `useStarterQuestions` is ON.
- **Docs restructure** — All documentation fragmented to stay under 25 KB per file. CHANGELOG split into archives.
